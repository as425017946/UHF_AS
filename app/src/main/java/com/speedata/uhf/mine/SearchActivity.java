...
//        SharedXmlUtil.getInstance(this).write("modle", "r2k");
        try {
            iuhfService = UHFManager.getUHFService(SearchActivity.this);
        } catch (Exception e) {
            e.printStackTrace();
        ...
        modle = SharedXmlUtil.getInstance(SearchActivity.this).read("modle", "");
//        initUI();
        newWakeLock();
    ...
    @BindView(R.id.search_duka)
    RelativeLayout edt_id;
    private void setinfo(){
        ...
        edt_id.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                SharedPFUtils.setParam(SearchActivity.this,"searchid",getIntent().getStringExtra("searchid"));
                if (getIntent().getStringExtra("searchid").equals("1")){
                    ...
                    //盘点选卡
                    final SearchTagDialog searchTag = new SearchTagDialog(SearchActivity.this, iuhfService, modle,R.style.DialogTheme);
                    searchTag.setTitle("读卡");
                    searchTag.setCancelable(true);
                    searchTag.setCanceledOnTouchOutside(true);
                    searchTag.show();
    ...
    private static final String[] list = {"EPC", "Reserved", "TID", "USER"};
